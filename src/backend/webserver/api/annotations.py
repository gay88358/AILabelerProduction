...
class Annotation(Resource):
    ...
    def get(self):
        ...
        return query_util.fix_ids(current_user.find_all_annotations())
    ...
    def post(self):
        ...
        image = current_user.find_image_by_id(image_id)
        if image is None:
            return {"message": "Invalid image id"}, 400
        
        logger.info(
            f'{current_user.get_username()} has created an annotation for image {image_id} with {isbbox}')
        logger.info(
            f'{current_user.get_username()} has created an annotation for image {image_id}')
...
class AnnotationId(Resource):
    ...
    def get(self, annotation_id):
        ...
        annotation = current_user.find_annotation_by_id(annotation_id)
    ...
    def delete(self, annotation_id):
        ...
        image = current_user.find_image_by_id(annotation.image_id)
    ...
    def put(self, annotation_id):
        ...
        logger.info(
            f'{current_user.get_username()} has updated category for annotation (id: {annotation.id})'
        )
        newAnnotation = current_user.find_annotation_by_id(annotation_id)
