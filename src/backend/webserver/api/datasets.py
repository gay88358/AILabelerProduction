...
class Dataset(Resource):
    ...
    def get(self):
        ...
        return query_util.fix_ids(current_user.find_all_datasets())

    @api.expect(Parser.create_dataset_parser())
    @login_required
    def post(self):
        """ Creates a dataset """
        args = Parser.create_dataset_parser().parse_args()
        name = args['name']
...
class DatasetId(Resource):
    ...
    @api.expect(Parser.update_dataset_parser())
    def post(self, dataset_id):
...
class DatasetData(Resource):
    ...
    #     datasets = current_user.find_all_exist_dataset()
    ...
    #         "categories": query_util.fix_ids(current_user.find_all_exist_categories())
    #     }
    ...
    def parse_dataset_folders(self, args):
        return args['dataset_folders'].split(",")
...
class DatasetCocoId(Resource):
    ...
    def get(self, import_id):
        ...
        coco_import = CocoImportModel.objects(
            id=import_id, creator=current_user.get_username()).first()
