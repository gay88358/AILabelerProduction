...
def authenticated_only(f):
    ...
    def wrapped(*args, **kwargs):
        if current_user.authenticated() or Config.LOGIN_DISABLED:
...
def annotating(data):
    ...
    if active:
        logger.info(f'{current_user.get_username()} has started annotating image {image_id}')
        ...
        if previous is not None:
            ...
            if previous_image is not None:
                ...
                previous_image.update(
                    pull__annotating=current_user.get_username()
                )
        ...
        image.update(add_to_set__annotating=current_user.get_username())
    else:
        leave_room(image_id)
...
@socketio.on('connect')
def connect():
    logger.info(f'Socket connection created with {current_user.get_username()}')


@socketio.on('disconnect')
def disconnect():
    if current_user.authenticated():
        logger.info(f'Socket connection has been disconnected with {current_user.get_username()}')
